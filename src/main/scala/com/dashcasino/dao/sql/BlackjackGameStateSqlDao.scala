...
class BlackjackGameStateSqlDao(implicit val session: AutoSession.type) {
  def toBlackjackGameState(rr: WrappedResultSet) = BlackjackGameState(rr.int("Id"), rr.int("GameId"), rr.string("UserHand"), rr.string("DealerHand"), rr.string("Description"), rr.int("LastCommandId"), rr.int("StatusCodeId"), rr.time("Timestamp").getTime)

  def insertBlackjackGameState(blackjackGameState: BlackjackGameState) =
    sql"INSERT INTO BlackjackGameState (GameId, UserHand, DealerHand, Description, LastCommandId, StatusCodeId, Timestamp) VALUES (${blackjackGameState.gameId}, ${blackjackGameState.userHand}, ${blackjackGameState.dealerHand}, ${blackjackGameState.description}, ${blackjackGameState.commandId}, ${blackjackGameState.statusCodeId}, CURRENT_TIMESTAMP)".update().apply()
